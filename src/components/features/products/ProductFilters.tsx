...
export default function ProductFilters({ onFilterChange }: ProductFiltersProps) {
  const [categories, setCategories] = useState<string[]>(["Panes", "Galletas"]);
  ...
  const handleCategoryChange = (category: string) => {
    ...
    onFilterChange?.({ categories: newCategories, priceRange, dietary });
  };
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            {["Panes", "Pasteles", "Galletas", "Postres", "Muffins"].map((category) => (
              <label key={category} className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-sm text-text-muted-light">{category}</span>
              </label>
            ))}
          </div>
        </div>
